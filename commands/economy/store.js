...
for (const file of itemFiles) {
	...
	// Push new item to collection itemsCollection & check if item is "restricted aka not sold"
	if (!item.restricted) itemsCollection.set(item.name, item);
}
...
module.exports = {
	...
	execute(message, args, bot) {
		if (!args.length) {
			let data = [];
			data.push(
				`${message.author}, there are all the items up for offer on the store:\n`
			);
		...
		if (!item)
			return message.channel.send(
				`${message.author}, try actually choosing an item this time...`
			);
